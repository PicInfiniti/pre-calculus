  ...
  <main>
    <section class="lesson-hero lesson-hero--information">
      ...
      <div class="information-hero-art" aria-hidden="true" data-reveal>
        ...
        <span class="information-hero-art__tag information-hero-art__tag--solve">${n(String.raw`f(x)=k`)}</span>
      ...
      <div class="projection-lab" data-reveal>
        ...
        <div class="projection-lab__controls">
          ...
          <div class="projection-facts">
            <article><span>Domain</span><strong>${n(String.raw`[-5,5]`)}</strong></article>
            <article><span>Range</span><strong>${n(String.raw`[-3,4]`)}</strong></article>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="lesson-section lesson-section--ink" id="level-lab">
      <div class="lesson-section__intro" data-reveal>
        ...
        <p>The equation ${n(String.raw`f(x)=k`)} asks where the curve meets ${n(String.raw`y=k`)}. For ${n(String.raw`f(x)>k`)}, keep the curve above the laser; for ${n(String.raw`f(x)<k`)}, keep the curve below it.</p>
      </div>
      ...
      <div class="level-lab" data-reveal>
        ...
        <div class="level-lab__controls">
          ...
          <div class="information-tabs information-tabs--dark" role="tablist" aria-label="Equation and inequality modes">
            <button type="button" class="is-active" data-level-relation="equal">${n(String.raw`f(x)=k`)}</button>
            <button type="button" data-level-relation="above">${n(String.raw`f(x)>k`)}</button>
            <button type="button" data-level-relation="below">${n(String.raw`f(x)<k`)}</button>
          </div>
      ...
      <div class="direction-lab" data-reveal>
        ...
        <div class="direction-lab__controls">
          ...
          <div class="direction-timeline" aria-label="Behavior intervals">
            <button type="button" data-direction-x="-4"><span>↑</span>${n(String.raw`(-5,-3)`)}</button>
            <button type="button" data-direction-x="-2"><span>↓</span>${n(String.raw`(-3,-1)`)}</button>
            <button type="button" data-direction-x="0"><span>↑</span>${n(String.raw`(-1,1.5)`)}</button>
            <button type="button" data-direction-x="2.2"><span>↓</span>${n(String.raw`(1.5,3)`)}</button>
            <button type="button" data-direction-x="4"><span>↑</span>${n(String.raw`(3,5)`)}</button>
          </div>
        </div>
      </div>
      ...
      <div class="radical-lab" data-reveal>
        ...
        <div class="radical-lab__controls">
          ...
          <div class="information-tabs" role="tablist" aria-label="Radical examples">
            <button type="button" class="is-active" data-radical="shifted">${n(String.raw`\sqrt{x+2}`)}</button>
            <button type="button" data-radical="semicircle">${n(String.raw`-\sqrt{25-x^2}`)}</button>
          </div>
    ...
    <section class="lesson-section readiness readiness--information" id="readiness">
      ...
      <div class="information-check-grid">
        ...
        <article class="information-check" data-reveal>
          ...
          <label><span class="information-check__prompt">Domain of ${n(String.raw`\sqrt{x+2}`)}</span>
          ...
          <label><span class="information-check__prompt">Range of ${n(String.raw`-\sqrt{25-x^2}`)}</span>
...
`,t(l);function u(e,t=2){let n=Number(e.toFixed(t)),r=a(n);return r.startsWith(`-`)?`−${r.slice(1)}`:r}function d(e,t=2){return u(e,t).replace(`−`,`-`)}function f({width:e,height:t,padding:n=48,xMin:r,xMax:i,yMin:a,yMax:o,equalScale:s=!1}){let c=(e-2*n)/(i-r),l=(t-2*n)/(o-a);s&&(c=l=Math.min(c,l));let u=(i-r)*c,d=(o-a)*l,f=(e-u)/2,p=(t-d)/2;return{width:e,height:t,xMin:r,xMax:i,yMin:a,yMax:o,left:f,right:f+u,top:p,bottom:p+d,x:e=>f+(e-r)*c,y:e=>p+(o-e)*l}}function p(e,{xStep:t=1,yStep:n=1,labelStep:r=2,light:i=!1}={}){let a=[];for(let n=Math.ceil(e.xMin/t)*t;n<=e.xMax+1e-8;n+=t)a.push(`<line x1="${e.x(n)}" y1="${e.top}" x2="${e.x(n)}" y2="${e.bottom}" class="info-grid-line ${i?`is-light`:``}" />`),n!==0&&Number.isInteger(n/r)&&a.push(`<text x="${e.x(n)}" y="${e.y(0)+20}" text-anchor="middle" class="info-grid-label ${i?`is-light`:``}">${u(n)}</text>`);for(let t=Math.ceil(e.yMin/n)*n;t<=e.yMax+1e-8;t+=n)a.push(`<line x1="${e.left}" y1="${e.y(t)}" x2="${e.right}" y2="${e.y(t)}" class="info-grid-line ${i?`is-light`:``}" />`),t!==0&&Number.isInteger(t/r)&&a.push(`<text x="${e.x(0)+10}" y="${e.y(t)-7}" class="info-grid-label ${i?`is-light`:``}">${u(t)}</text>`);return e.yMin<=0&&e.yMax>=0&&a.push(`<line x1="${e.left}" y1="${e.y(0)}" x2="${e.right}" y2="${e.y(0)}" class="info-axis-line ${i?`is-light`:``}" />`),e.xMin<=0&&e.xMax>=0&&a.push(`<line x1="${e.x(0)}" y1="${e.top}" x2="${e.x(0)}" y2="${e.bottom}" class="info-axis-line ${i?`is-light`:``}" />`),a.join(``)}function m(e,t){return e.map(([e,n],r)=>`${r?`L`:`M`}${t.x(e)} ${t.y(n)}`).join(` `)}function h(e,t,n,r=.04){let i=[];for(let a=e;a<t;a+=r)i.push([a,n(a)]);return i.push([t,n(t)]),i}var g=[{x:-5,y:-3},{x:-3,y:3,kind:`local maximum`},{x:-1,y:0,kind:`local minimum`},{x:1.5,y:2,kind:`local maximum`},{x:3,y:-1,kind:`local minimum`},{x:5,y:4}];function _(e){for(let t=0;t<g.length-1;t+=1)if(e<=g[t+1].x+1e-9)return t;return g.length-2}function v(e){let t=_(Math.max(-5,Math.min(5,e))),n=g[t],r=g[t+1],i=(e-n.x)/(r.x-n.x),a=i*i*(3-2*i);return n.y+(r.y-n.y)*a}function y(e){if(g.find(t=>i(e,t.x,.001)))return 0;let t=_(e),n=g[t],r=g[t+1],a=(e-n.x)/(r.x-n.x);return(r.y-n.y)*6*a*(1-a)/(r.x-n.x)}var b=h(-5,5,v,.025),x=f({width:680,height:500,padding:27.5,xMin:-6,xMax:6,yMin:-4,yMax:5});function S({light:e=!1}={}){return`
  ...
  `}var C=document.querySelector(`#projection-chart`),w=document.querySelector(`#projection-x`),T=`input`;function E(){let t=Number(w.value),n=v(t),r=`
    <line x1="${x.x(t)}" y1="${x.y(0)}" x2="${x.x(t)}" y2="${x.y(n)}" class="projection-line" />
    <line x1="${x.x(0)}" y1="${x.y(n)}" x2="${x.x(t)}" y2="${x.y(n)}" class="projection-line projection-line--soft" />
    <circle cx="${x.x(t)}" cy="${x.y(n)}" r="9" class="info-focus-point" />
  `,i=`At this input`,a=String.raw`f(${d(t,1)})\approx ${d(n,1)}`,o=`Read vertically from x = ${u(t,1)} until you meet the curve.`;T===`domain`?(r=`<line x1="${x.x(-5)}" y1="${x.y(0)}" x2="${x.x(5)}" y2="${x.y(0)}" class="projection-shadow projection-shadow--domain" />`,i=`Horizontal shadow`,a=String.raw`D_f=[-5,5]`,o=`Both endpoints are filled, so −5 and 5 are included.`):T===`range`&&(r=`<line x1="${x.x(0)}" y1="${x.y(-3)}" x2="${x.x(0)}" y2="${x.y(4)}" class="projection-shadow projection-shadow--range" />`,i=`Vertical shadow`,a=String.raw`R_f=[-3,4]`,o=`The curve reaches every height from its lowest to its highest output.`),C.innerHTML=`${S()}${r}`,document.querySelector(`#projection-x-output`).textContent=u(t,1),document.querySelector(`#projection-result-label`).textContent=i,e(document.querySelector(`#projection-result-main`),a),document.querySelector(`#projection-result-copy`).textContent=o,w.disabled=T!==`input`}document.querySelectorAll(`[data-projection-mode]`).forEach(e=>e.addEventListener(`click`,()=>{T=e.dataset.projectionMode,document.querySelectorAll(`[data-projection-mode]`).forEach(t=>t.classList.toggle(`is-active`,t===e)),E()})),w.addEventListener(`input`,E),E();var D=document.querySelector(`#level-chart`),O=document.querySelector(`#level-y`),k=`equal`;function A(e){let t=[];for(let n=0;n<g.length-1;n+=1){let r=g[n],a=g[n+1];if(e<Math.min(r.y,a.y)-1e-8||e>Math.max(r.y,a.y)+1e-8||(i(e,r.y)&&!t.some(e=>i(e,r.x))&&t.push(r.x),i(e,a.y)&&!t.some(e=>i(e,a.x))&&t.push(a.x),i(r.y,a.y)||i(e,r.y)||i(e,a.y)))continue;let o=r.x,s=a.x;for(let t=0;t<40;t+=1){let t=(o+s)/2;(v(o)-e)*(v(t)-e)<=0?s=t:o=t}let c=(o+s)/2;t.some(e=>i(e,c,.01))||t.push(c)}return t.sort((e,t)=>e-t)}function j(e,t){let n=h(-5,5,v,.025),r=[],i=[];return n.forEach(n=>{(t===`above`?n[1]>e:n[1]<e)?i.push(n):i.length&&(r.push(i),i=[])}),i.length&&r.push(i),r}function M(e,t){let n=[-5,...A(e),5],r=[];for(let a=0;a<n.length-1;a+=1){let o=n[a],s=n[a+1],c=(o+s)/2;if((t===`above`?v(c)>e:v(c)<e)&&s-o>.01){let n=i(o,-5)&&(t===`above`?v(-5)>e:v(-5)<e)?`[`:`(`,a=i(s,5)&&(t===`above`?v(5)>e:v(5)<e)?`]`:`)`;r.push(`${n}${u(o,1)}, ${u(s,1)}${a}`)}}return r.join(` ∪ `)||`no x-values`}function N(){let t=Number(O.value),n=A(t),r=k===`equal`?``:j(t,k).map(e=>`<path d="${m(e,x)}" class="level-highlight" />`).join(``);if(D.innerHTML=`
    ${S()}
    <line x1="${x.left}" y1="${x.y(t)}" x2="${x.right}" y2="${x.y(t)}" class="level-laser" />
    ${r}
    ${n.map(e=>`<circle cx="${x.x(e)}" cy="${x.y(t)}" r="8" class="level-crossing" />`).join(``)}
  `,document.querySelector(`#level-y-output`).textContent=u(t,1),k===`equal`){document.querySelector(`#level-result-label`).textContent=`${n.length} crossing${n.length===1?``:`s`}`;let t=document.querySelector(`#level-result-main`);n.length?e(t,String.raw`x\approx ${n.map(e=>d(e,1)).join(`, `)}`):t.textContent=`No solution at this level`,document.querySelector(`#level-result-copy`).textContent=`Each crossing contributes one solution to f(x) = k.`}else{let n=k===`above`?`>`:`<`;document.querySelector(`#level-result-label`).textContent=k===`above`?`Curve above the laser`:`Curve below the laser`,e(document.querySelector(`#level-result-main`),String.raw`f(x)${n}${d(t,1)}`),document.querySelector(`#level-result-copy`).textContent=`Approximately ${M(t,k)}`}}document.querySelectorAll(`[data-level-relation]`).forEach(e=>e.addEventListener(`click`,()=>{k=e.dataset.levelRelation,document.querySelectorAll(`[data-level-relation]`).forEach(t=>t.classList.toggle(`is-active`,t===e)),N()})),O.addEventListener(`input`,N),N();var P=document.querySelector(`#direction-chart`),F=document.querySelector(`#direction-x`);function I(){let e=Number(F.value),t=v(e),n=y(e),r=g.slice(1,-1).find(t=>i(e,t.x,.045)),a=1.05,o=[[e-a,t-n*a],[e+a,t+n*a]];P.innerHTML=`
    ${S({light:!0})}
